...
def updateFromZip(message=config.get_localized_string(80050)):
    ...
    extractedDir = filetools.join(destpathname, "myaddon-" + branch)
    #extractedDir = filetools.join(destpathname, f"myaddon-{branch}-plugin.video.lo-scienziato-pazzo")
...
def download():
    ...
    localCommitFile.write(str(hash+'\n').encode('utf-8'))
    localCommitFile.write(str(last_commit).encode('utf-8'))
...
def run():
    if chooseBranch():
        ...
        t.join()
        refreshLang()
        platformtools.dialog_ok(heading='Branch details',line1='Selected Branch: '+branch,line2='Number Of commits:'+str(branch_commits),line3='last commit:'+last_commit)
